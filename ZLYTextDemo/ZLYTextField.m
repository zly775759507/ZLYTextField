...
- (void)addTextChangeObserver{
    self.isEmoticons = NO;
    self.LimiType = 1;
    self.textPoint = CGPointMake(8, 8);
    self.delegate = self;
    self.isFirst = YES;
...
{
    if (self.returnDelegate &&[self.returnDelegate respondsToSelector:@selector(textFieldReturnKeyPress:)]) {
        [self.returnDelegate textFieldReturnKeyPress:self];
    }
    return YES;
}
